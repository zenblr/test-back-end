...
export class EmiDetailsService {
  ...
  getAllEmiDetails(from?, to?, search?): Observable<any> {
    return this.http.get<any>(`http://173.249.49.7:9120/api/emi-details?search=${search}&from=${from}&to=${to}`);
  }

  reportExport(): Observable<any> {
    return this.http.get(`http://173.249.49.7:9120/api/emi-details/emi-report`, { responseType: 'arraybuffer' })
      .pipe(
        map((res) => {
          return res;
        }),
