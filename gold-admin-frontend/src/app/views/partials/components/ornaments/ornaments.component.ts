  ...
  calculateTotal() {
    ...
    this.OrnamentsData.value.forEach(gross => {
      if (gross.grossWeight) {
        this.totalGrossWeight += Number(Number(gross.grossWeight).toFixed(2))
      }
      if (gross.netWeight) {
        this.totalNetWeight += Number(Number(gross.netWeight).toFixed(2))
      }
      if (gross.deductionWeight) {
        this.totalDeductionWeight +=  Number(Number(gross.deductionWeight).toFixed(2))
      }
      if (gross.ltvPercent && gross.netWeight) {
        this.totalPurtiy +=  Number(Number(gross.ltvPercent/100 * gross.netWeight).toFixed(2))
      }
    })
  }
