...
exports.AddBranch = async (req, res) => {
  try {
    ...
    await sequelize
      .transaction(async t => {
        let addbranch = await models.branch.create(
          {
            partnerId,
            name,
            cityId,
        ...
        let partnerdataid = await models.partner.findOne({
          where: { id: addbranch.dataValues.partnerId },
          transaction: t
        });
        ...
        let newId =
          pqid.slice(0, 2) +
          addbranch.dataValues.name.slice(0, 3).toUpperCase() +
          "-" +
          id;
...
exports.UpdateBranch = async (req, res) => {
  ...
  let branchdata = await models.branch.update(
    {
      partnerId,
      branchId: pId,
      name,
