...
exports.loanBasicDeatils = async (req, res, next) => {
    ...
    let loanData = await sequelize.transaction(async t => {
        let masterLoan;
        if (partReleaseId) {
            masterLoan = await models.customerLoanMaster.create({ customerId: customerId, loanStageId: stageId.id, customerLoanCurrentStage: '2', internalBranchId: req.userData.internalBranchId, createdBy, modifiedBy, isNewLoanFromPartRelease: true }, { transaction: t })
        } else {
            masterLoan = await models.customerLoanMaster.create({ customerId: customerId, loanStageId: stageId.id, customerLoanCurrentStage: '2', internalBranchId: req.userData.internalBranchId, createdBy, modifiedBy }, { transaction: t })
        }

        await models.appraiserRequest.update({ status: 'complete' }, { where: { id: requestId }, transaction: t })

        let masterLoan = await models.customerLoanMaster.create({ customerId: customerId, loanStageId: stageId.id, customerLoanCurrentStage: '2', internalBranchId: req.userData.internalBranchId, appraiserRequestId: requestId, createdBy, modifiedBy }, { transaction: t })
        ...
        let loanData = await sequelize.transaction(async t => {
            ...
            if (isUnsecuredSchemeApplied == true) {
                ...
                if (getUnsecuredLoanId.unsecuredLoanId == null) {
                    ...
                    var unsecuredLoan = await models.customerLoan.create({ customerId: loanSubmitted.customerId, masterLoanId, partnerId, schemeId: unsecuredSchemeId, loanAmount: unsecuredLoanAmount, outstandingAmount: unsecuredLoanAmount, interestRate: unsecuredInterestRate, penalInterest: unsecuredPenal.penalInterest, currentSlab: paymentFrequency, selectedSlab: paymentFrequency, currentInterestRate: unsecuredInterestRate, loanType: "unsecured", createdBy, modifiedBy }, { transaction: t })
        ...
        await sequelize.transaction(async (t) => {
            ...
            if (loanMaster.isLoanTransfer == true || loanMaster.isNewLoanFromPartRelease == true) {
                ...
                if (loan.isUnsecuredSchemeApplied) {
                    let unsecuredDisbursed = await models.customerTransactionDetail.create({ masterLoanId, loanId: loan.customerLoan[1].id, debit: loan.customerLoan[1].loanAmount, description: `Loan amount disbursed to customer` }, { transaction: t })
