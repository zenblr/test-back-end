...
exports.checkPartAmount = async (req, res, next) => {
    ...
    if (loan.loan.customerLoan.length > 1) {
        var paidAmountSplitUp = await splitAmountIntoSecuredAndUnsecured(loan.loan, paidAmount)
    }else{
        paidAmountSplitUp = paidAmount;
    }
    ...
    // data.securedRatio = paidAmountSplitUp.securedRatio
    // data.unsecuredRatio = paidAmountSplitUp.unsecuredRatio
    return res.status(200).json({ data, paidAmountSplitUp })
}
