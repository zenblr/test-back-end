...
exports.refundCron = async (req, res, next) => {
    ...
    await sequelize.transaction(async (t) => {
        ...
        // let digiGoldWalletTemp = await models.walletTransactionTempDetails.findAll({ where: { isOrderPlaced: false, razorPayTransactionId: { [Op.not]: null }, refundCronExecuted: false, createdAt: { [Op.lt]: moment().subtract(2, 'days').format('YYYY-MM-DD') } }, attributes: [ 'id', 'isOrderPlaced', 'razorPayTransactionId', 'refundCronExecuted'], transaction: t });
        ...
        for (const temp of loanRazorpayTemp) {
            ...
            if (razerpayInfo) {
                ...
                //     if (customerTransaction.ornamentId.length == ornaments.length) {
