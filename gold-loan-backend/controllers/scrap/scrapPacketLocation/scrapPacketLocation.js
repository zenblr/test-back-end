...
exports.getPacketLocation = async (req, res, next) => {
    ...
    let allPocket = await models.scrapPacketLocation.findAll(whereCondition)
    let count = await models.scrapPacketLocation.findAll({
        where: searchQuery,
      });
...
exports.updatePacketLocation = async (req, res, next) => {
    ...
    let pocketExist = await models.scrapPacketLocation.findOne({ where: { location: location, isActive: true } })
    if (!check.isEmpty(pocketExist)) {
        return res.status(404).json({ message: 'This Packet Location already Exists' });
    }
    let UpdateData = await models.scrapPacketLocation.update({ location }, { where: { id: id } })
    if (UpdateData[0] === 0) {
...
exports.deactivatePacketLoaction = async (req, res, next) => {
    ...
    const pocket = await models.scrapPacketLocation.update({ isActive: isActive }, { where: { id: id } })
    if (pocket[0] == 0) {
        return res.status(404).json({ message: "pocket location failed to delete" });
    }
    return res.status(200).json({ message: `Success` })
