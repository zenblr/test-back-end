...
let mergeInterestTable = async (masterLoanId) => {
    ...
    for (let i = 0; i < securedTable.length; i++) {
        ...
        if (interestTable.isUnsecuredSchemeApplied) {
            ...
            data.penalPaid = Number(securedTable[i].penalPaid) + Number(unsecuredTable[i].penalPaid)
        ...
        for (const loan of loanInfo) {
            ...
            if (noOfDays > loan.currentSlab && checkDueDateForSlab) {
                ...
                for (const interestData of interestLessThanDate) {
                    let checkDebitEntry = await models.customerTransactionDetail.findAll({ where: { masterLoanId: loan.masterLoanId, loanId: loan.id, loanInterestId: interestData.id,credit:null} });
